Feature:
  ...
  Scenario: End to End Scenario
    ...
    And user selects a company and accepts warning
    And user selects Menu tab from Home Page
    And user Navigate to Global Settings
    And user clicks on Custom Stylesheet
    ...
    And user add header title as test and insert barcode
    ...
    And user add table and tableName as test
    And user navigate to body Configuration
    And user selects feedback
    And user navigate to static statement 
    And user add certification statement as test certificate and click on save
